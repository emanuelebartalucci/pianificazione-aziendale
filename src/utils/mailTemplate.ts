...
export function wrapMailTemplate(title: string, htmlContent: string): string {
  ...
  // Se il contenuto è già una scheda ricca completa (es. Apertura/Chiusura Commessa o layout con card integrata)
  const isFullCard = cleanedContent.includes('max-width:') || cleanedContent.includes('Scheda Apertura') || cleanedContent.includes('Scheda Chiusura');
...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <body style="margin: 0; padding: 24px 12px; background-color: #f1f5f9; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; -webkit-text-size-adjust: 100%; -ms-text-size-adjust: 100%;">
    <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color: #f1f5f9; width: 100%;">
      <tr>
        <td align="center">
          <div style="max-width: 720px; width: 100%; margin: 0 auto; text-align: left;">
            ${cleanedContent}
          </div>
          
          <!-- Footer Unificato -->
          <table width="100%" border="0" cellspacing="0" cellpadding="0" style="max-width: 720px; width: 100%; text-align: center; margin-top: 16px;">
            <tr>
              <td style="padding: 12px 20px; font-family: Arial, Helvetica, sans-serif; font-size: 11px; line-height: 1.5; color: #94a3b8; text-align: center;">
